...
async function eliminarAlumno(formData) {
    ...
    revalidatePath("/alumnos-api") //para que se actualice la pagina
}
                        ...
                        rows.map((alumno) => (
                            <div key={alumno.id} className="p-4 bg-white shadow-sm rounded-md flex justify-between items-center">
                                ...
                                <div className="flex space-x-2">
                                    <Link href={`/alumnos/${alumno.id}`} className="text-blue-500 hover:underline">Ver</Link>
                                    <Link href={`/alumnos/${alumno.id}/modificar`} className="text-yellow-500 hover:underline">Modificar</Link>
